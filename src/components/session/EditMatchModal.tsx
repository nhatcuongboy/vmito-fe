...
export function EditMatchModal({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedPlayerIds.length === 4 && selectedPlayerIds.every(id => id)) {
    ...
    <Modal isOpen={isOpen} onClose={onClose} size="lg">
      ...
      <ModalContent>
        ...
        <ModalBody>
          <VStack spacing={4} align="stretch">
            <FormControl>
              ...
              <HStack>
                <VStack>
                    <Text fontSize="sm" fontWeight="bold" color="blue.600">Pair 1</Text>
                    <Input 
                        value={pair1Score} 
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setPair1Score(e.target.value)} 
                        type="number" 
                    />
                </VStack>
                <Text fontWeight="bold">-</Text>
                <VStack>
                    <Text fontSize="sm" fontWeight="bold" color="red.600">Pair 2</Text>
                    <Input 
                        value={pair2Score} 
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setPair2Score(e.target.value)} 
                        type="number" 
                    />
                </VStack>
              </HStack>
              ...
              <VStack spacing={2}>
                 <HStack width="100%" spacing={2}>
                    <VStack flex={1} spacing={2}>
                        <Text fontSize="xs" color="gray.500">Pair 1 (Position 1)</Text>
                        <Select 
                            value={selectedPlayerIds[0] || ''} 
                            onChange={(e: ChangeEvent<HTMLSelectElement>) => handlePlayerChange(0, e.target.value)}
                        >
                            <option value="" disabled>Select player</option>
                            {players.map(p => (
                                <option key={p.id} value={p.id}>#{p.playerNumber} {p.name}</option>
                            ))}
                        </Select>
                        <Text fontSize="xs" color="gray.500">Pair 1 (Position 2)</Text>
                         <Select 
                            value={selectedPlayerIds[1] || ''} 
                            onChange={(e: ChangeEvent<HTMLSelectElement>) => handlePlayerChange(1, e.target.value)}
                        >
                            <option value="" disabled>Select player</option>
                             {players.map(p => (
                                <option key={p.id} value={p.id}>#{p.playerNumber} {p.name}</option>
                     ...
                     <VStack flex={1} spacing={2}>
                        <Text fontSize="xs" color="gray.500">Pair 2 (Position 3)</Text>
                        <Select 
                            value={selectedPlayerIds[2] || ''} 
                            onChange={(e: ChangeEvent<HTMLSelectElement>) => handlePlayerChange(2, e.target.value)}
                        >
                            <option value="" disabled>Select player</option>
                             {players.map(p => (
                                <option key={p.id} value={p.id}>#{p.playerNumber} {p.name}</option>
                        ...
                        <Text fontSize="xs" color="gray.500">Pair 2 (Position 4)</Text>
                         <Select 
                            value={selectedPlayerIds[3] || ''} 
                            onChange={(e: ChangeEvent<HTMLSelectElement>) => handlePlayerChange(3, e.target.value)}
                        >
                            <option value="" disabled>Select player</option>
                             {players.map(p => (
                                <option key={p.id} value={p.id}>#{p.playerNumber} {p.name}</option>
        ...
        <ModalFooter>
          ...
          <Button colorScheme="blue" onClick={handleSubmit} {...({ isLoading: isSubmitting } as any)}>
            Save Changes
          </Button>
        </ModalFooter>
      </ModalContent>
